...
//show lifes
    lifeTiles[i] = new game.tile( IMAGE_DIR + 'hp.png', 30, 30);
    lifeTiles[i].setPosition(5+32*i, 5, Z_GUI);
}

for (i=0;i<5;i++) {
//display points
    pointsTiles[i] = new game.tile( IMAGE_DIR + 'interface/all.png', 31, 41);
...
var lifeDown = function() {
    ...
    if (actualLifes>1) {
        ...
        newLocation.pop();
        newLocation.push('odyssey/lose');
        window.location = newLocation.join('/');*/
        $("#odyssey-lose").show(750);
        $("canvas").hide();
        CallShop();
    }
}
